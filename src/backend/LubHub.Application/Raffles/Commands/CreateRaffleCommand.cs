...
namespace LubHub.Application.Raffles.Commands
{
    ...
    public record CreateRaffleCommand(int StreamerId, string Title) : IRequest<int>;
    ...
    public class CreateRaffleCommandHandler(IRaffleRepository raffleRepository, IStreamerRepository streamerRepository) : IRequestHandler<CreateRaffleCommand, int>
    {
        ...
        {
            var streamer = await streamerRepository.GetByIdAsync(request.StreamerId, cancellationToken)
                ?? throw new NotFoundException(nameof(Streamer), request.StreamerId);
            ...
            var raffle = Raffle.Create(request.StreamerId, request.Title);
